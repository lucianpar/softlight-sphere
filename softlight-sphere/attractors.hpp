...
struct Attractor {
    ...
    al::VAOMesh makeNoiseCube(al::VAOMesh& mMesh, float radius = 1.0, int nVertices = 100) {
    ...
    void processChen(al::VAOMesh& mMesh, float time, float speed, float a = 35.0f, float c = 28.0f, float d = 3.0f) {
    auto& vertices = mMesh.vertices();
    ...
    void processLorenz(al::VAOMesh& mMesh, float time, float speed, float sigma = 10.0f, float rho = 28.0f, float beta = 8.0f / 3.0f) {
    auto& vertices = mMesh.vertices();
    for (int i = 0; i < vertices.size(); ++i) {
        float x = vertices[i].x;
        float y = vertices[i].y;
    ...
    void processDadras(al::VAOMesh& mMesh, float time, float speed, float a = 3.0f, float b = 2.7f, float c = 1.7f, float d = 2.0f, float e = 9.0f) {
    auto& vertices = mMesh.vertices();
    for (int i = 0; i < vertices.size(); ++i) {
        float x = vertices[i].x;
        float y = vertices[i].y;
    ...
    void processTSUCS(al::VAOMesh& mMesh, float time, float speed, float a = 40.0f, float b = 0.833f, float c = 0.5f, float d = 20.0f, float e = 0.65f) {
    auto& vertices = mMesh.vertices();
    for (int i = 0; i < vertices.size(); ++i) {
        float x = vertices[i].x;
        float y = vertices[i].y;
    ...
    void processRabinovich(al::VAOMesh& mMesh, float time, float speed, float a = 0.14f, float b = 0.10f) {
    auto& vertices = mMesh.vertices();
    for (int i = 0; i < vertices.size(); ++i) {
        float x = vertices[i].x;
        float y = vertices[i].y;
